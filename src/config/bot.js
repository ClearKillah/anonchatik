...
module.exports = (app) => {
  ...
  bot.start(async (ctx) => {
    try {
      ...
      ctx.reply(
        ...
        'Нажмите на кнопку ниже, чтобы открыть анонимный чат и начать общение с незнакомцами.',
        webAppButton
      );
    } catch (error) {
  ...
  bot.help((ctx) => {
    ctx.reply(
      'Этот бот позволяет анонимно общаться с другими пользователями.\n\n' +
      'Доступные команды:\n' +
      '/start - начать использование бота\n' +
      '/help - показать справку\n\n' +
      'Нажмите на кнопку "Открыть анонимный чат", чтобы начать общение.'
    );
  });
